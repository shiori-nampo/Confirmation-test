    ...
    <div class="admin-table">
        <table class="admin-table__inner">
            ...
            <tr class="admin-table__row">
                <th class="admin-table__header admin-table__header-name">
                    お名前
                </th>
                <th class="admin-table__header admin-table__header-gender">
                    性別
                </th>
                <th class="admin-table__header admin-table__header-email">
                    メールアドレス
                </th>
                <th class="admin-table__header admin-table__header-category">
                    お問い合わせの種類
                </th>
                <th class="admin-table__header admin-table__header-detail"></th>
            </tr>
            </thead>
            <tbody>
                @foreach($contacts as $contact)
                <tr class="admin-table__row">
                    <td class="admin-table__item admin-table__item-name">
                        {{ $contact->last_name }} {{ $contact->first_name }}
                    </td>
                    <td class="admin-table__item admin-table__item-gender">
                        {{ $contact->gender == 1 ? '男性' : '女性' }}
                    </td>
                    <td class="admin-table__item admin-table__item-email">
                        {{ $contact->email }}
                    </td>
                    <td class="admin-table__item admin-table__item-category">
                        {{ $contact->category->content ?? '未選択' }}
                    </td>
                    <td class="admin-table__item admin-table__item-detail">
